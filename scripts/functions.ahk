...
GameLoaded() {
    ...
    loop {
        ...
        ; Detect "Connecting to Roblox" error dialog -- this is a modal popup
        ; that keeps the process alive but never loads the game. Close it and
        ; return 0 so the caller retries the join immediately.
        if WinExist("Connecting to Roblox") or WinExist("Roblox ahk_exe RobloxPlayerBeta.exe", "Error") {
            PlayerStatus("Connecting to Roblox error -- closing and retrying", "0xfa7900", ,false, ,false)
            Gdip_DisposeImage(pBMScreen)
            CloseRoblox()
            Sleep 2000
            return 0
...
detectViciousDefeated() {
    static lastOCR := 0
    if (A_TickCount - lastOCR > 5000) {
        lastOCR := A_TickCount
        pBMScreen := GetpBMScreen(windowX + windowWidth - 500, windowY + 175, 500, 100)
        ; Check for "defeated" + "vicious" + "been" in same OCR text
        ; "Defeated Werewolf!" / "Defeated Scorpion!" have "defeated" but no "vicious" or "been"
        ; Real VB defeat: "Vicious Bee has been defeated"
        if (OCR_CheckKeyword(pBMScreen, "defeated") && OCR_CheckKeyword(pBMScreen, "vicious") && OCR_CheckKeyword(pBMScreen, "been")) {
            Gdip_DisposeImage(pBMScreen)
            return true
        }
        if (OCR_CheckKeyword(pBMScreen, "from vicious bee")) {
            Gdip_DisposeImage(pBMScreen)
            return true
        }
        Gdip_DisposeImage(pBMScreen)
...
detectViciousLeft(){
    ...
    if (A_TickCount - lastOCR > 5000) {
        lastOCR := A_TickCount
        pBMScreen := GetpBMScreen(windowX + windowWidth - 500, windowY + 175, 500, 100)
        ocrResult := OCR_CheckKeyword(pBMScreen, "left")
        Gdip_DisposeImage(pBMScreen)
        if (ocrResult)
            return true
    }
...
; ============================================================================
; OCR_CheckKeyword: checks if a keyword appears in the chat area
; Uses same WinRT OCR as OCR_DetectField but just checks for a keyword.
; Returns true if found, false otherwise.
; ============================================================================
OCR_CheckKeyword(pBitmap, keyword) {
    ; Upscale 2x
    srcW := 0, srcH := 0
    Gdip_GetImageDimensions(pBitmap, &srcW, &srcH)
    pUpscaled := Gdip_CreateBitmap(srcW * 3, srcH * 3)
    pGraphics := Gdip_GraphicsFromImage(pUpscaled)
    Gdip_SetInterpolationMode(pGraphics, 7)
    Gdip_DrawImage(pGraphics, pBitmap, 0, 0, srcW * 3, srcH * 3, 0, 0, srcW, srcH)
    Gdip_DeleteGraphics(pGraphics)

    tmpPath := A_Temp "\vichop_ocr_kw.png"
    Gdip_SaveBitmapToFile(pUpscaled, tmpPath)
    Gdip_DisposeImage(pUpscaled)

    ps1Path := A_Temp "\vichop_ocr_kw.ps1"
    try FileDelete ps1Path
    ps1Content := "Add-Type -AssemblyName System.Runtime.WindowsRuntime`n"
    ps1Content .= "[Windows.Storage.StorageFile, Windows.Storage, ContentType = WindowsRuntime] | Out-Null`n"
    ps1Content .= "[Windows.Media.Ocr.OcrEngine, Windows.Media.Ocr, ContentType = WindowsRuntime] | Out-Null`n"
    ps1Content .= "[Windows.Graphics.Imaging.BitmapDecoder, Windows.Graphics.Imaging, ContentType = WindowsRuntime] | Out-Null`n"
    ps1Content .= "function Await($t, $r) { $m = ([System.WindowsRuntimeSystemExtensions].GetMethods() | ? { $_.Name -eq 'AsTask' -and $_.GetParameters().Count -eq 1 -and $_.GetParameters()[0].ParameterType.Name -eq 'IAsyncOperation``1' })[0]; $a = $m.MakeGenericMethod($r); $n = $a.Invoke($null, @($t)); $n.Wait(-1) | Out-Null; $n.Result }`n"
    ps1Content .= "$f = Await ([Windows.Storage.StorageFile]::GetFileFromPathAsync('" tmpPath "')) ([Windows.Storage.StorageFile])`n"
    ps1Content .= "$s = Await ($f.OpenAsync([Windows.Storage.FileAccessMode]::Read)) ([Windows.Storage.Streams.IRandomAccessStream])`n"
    ps1Content .= "$d = Await ([Windows.Graphics.Imaging.BitmapDecoder]::CreateAsync($s)) ([Windows.Graphics.Imaging.BitmapDecoder])`n"
    ps1Content .= "$b = Await ($d.GetSoftwareBitmapAsync()) ([Windows.Graphics.Imaging.SoftwareBitmap])`n"
    ps1Content .= "$e = [Windows.Media.Ocr.OcrEngine]::TryCreateFromUserProfileLanguages()`n"
    ps1Content .= "$r = Await ($e.RecognizeAsync($b)) ([Windows.Media.Ocr.OcrResult])`n"
    ps1Content .= "Write-Output $r.Text"
    FileAppend ps1Content, ps1Path

    ocrText := ""
    try {
        wsh := ComObject("WScript.Shell")
        exec := wsh.Exec('powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File "' ps1Path '"')
    ...
    return InStr(StrLower(ocrText), StrLower(keyword))
}

; ============================================================================
; OCR-based field detection using Windows.Media.Ocr (WinRT)
; No external dependencies. Works on Windows 10+.
; Returns the detected field name, or "" if no field found.
; ============================================================================
OCR_DetectField(pBitmap) {
    ; === DEBUG LOGGING ===
    dbgPath := A_ScriptDir "\ocr_debug.log"
    try FileDelete dbgPath
    dbgLine(s) {
        FileAppend s "`n", dbgPath
    }

    ; Upscale the bitmap 2x for better OCR accuracy on small text
    srcW := 0, srcH := 0
    Gdip_GetImageDimensions(pBitmap, &srcW, &srcH)
    dbgLine("[OCR] Source bitmap: " srcW "x" srcH)

    pUpscaled := Gdip_CreateBitmap(srcW * 3, srcH * 3)
    if !pUpscaled {
        dbgLine("[OCR] ERROR: Gdip_CreateBitmap failed")
        return ""
    }
    pGraphics := Gdip_GraphicsFromImage(pUpscaled)
    Gdip_SetInterpolationMode(pGraphics, 7)
    Gdip_DrawImage(pGraphics, pBitmap, 0, 0, srcW * 3, srcH * 3, 0, 0, srcW, srcH)
    Gdip_DeleteGraphics(pGraphics)
    dbgLine("[OCR] Upscaled to: " srcW*3 "x" srcH*3)

    ; Save the upscaled bitmap to a temp file for OCR
    tmpPath := A_Temp "\vichop_ocr_temp.png"
    Gdip_SaveBitmapToFile(pUpscaled, tmpPath)
    Gdip_DisposeImage(pUpscaled)
    dbgLine("[OCR] Saved temp PNG: " tmpPath " (exists: " FileExist(tmpPath) ")")

    ; Write the OCR PowerShell script to a temp .ps1 file
    ps1Path := A_Temp "\vichop_ocr_cmd.ps1"
    try FileDelete ps1Path
    ps1Content := "Add-Type -AssemblyName System.Runtime.WindowsRuntime`n"
    ps1Content .= "[Windows.Storage.StorageFile, Windows.Storage, ContentType = WindowsRuntime] | Out-Null`n"
    ps1Content .= "[Windows.Media.Ocr.OcrEngine, Windows.Media.Ocr, ContentType = WindowsRuntime] | Out-Null`n"
    ...
    ps1Content .= "Write-Output $r.Text"
    FileAppend ps1Content, ps1Path
    dbgLine("[OCR] Wrote PS1 script: " ps1Path)

    ; Run PowerShell and capture stdout via WScript.Shell
    ocrText := ""
    try {
        wsh := ComObject("WScript.Shell")
        exec := wsh.Exec('powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File "' ps1Path '"')
        ...
        dbgLine("[OCR] PowerShell stdout: " SubStr(ocrText, 1, 200))
    } catch as e {
        dbgLine("[OCR] ERROR running PowerShell: " e.Message)
        ocrText := ""
    }
    ...
    try {
        ...
        if (errText != "")
            dbgLine("[OCR] PowerShell stderr: " SubStr(errText, 1, 300))
    } catch {
        dbgLine("[OCR] Could not read stderr")
    }
    ...
    ocrText := Trim(ocrText)
    dbgLine("[OCR] Final text (trimmed): [" ocrText "]")

    if (ocrText = "") {
        dbgLine("[OCR] No text returned")
        return ""
    }

    ; Extract field name from VB notification text
    ; BSS format: "Vicious Bee is attacking [player] in the [Field]!"
    ; But OCR may garble "attacking [player]" into other words.
    ; Strategy: require "vicious" in text, then find "in the " + field suffix.
    ; The "vicious" requirement prevents false positives from honey counters
    ; and other chat messages that mention field names.
    fields := ["pepper", "mountain", "cactus", "rose", "spider", "clover"]

    if !InStr(ocrText, "vicious") {
        dbgLine("[OCR] No 'vicious' keyword found -- not a VB notification")
        return ""
    }

    ; Look for "in the X [Field/Patch/Top]" pattern anywhere in the text
    searchPos := 1
    while (inThePos := InStr(ocrText, "in the ",, searchPos)) > 0 {
        fieldStart := inThePos + 7  ; skip "in the "
        ; Try " Field" first, then " Patch", then " Top"
        fieldEnd := InStr(ocrText, " Field",, fieldStart)
        if (fieldEnd <= 0)
            fieldEnd := InStr(ocrText, " Patch",, fieldStart)
        if (fieldEnd <= 0)
            fieldEnd := InStr(ocrText, " Top",, fieldStart)
        if (fieldEnd > 0) {
            extractedField := SubStr(ocrText, fieldStart, fieldEnd - fieldStart)
            extractedField := Trim(extractedField)
            ; Match against known fields (case-insensitive)
            for field in fields {
                if InStr(StrLower(extractedField), field) {
                    dbgLine("[OCR] MATCHED: " field " (via 'in the X' extraction: " extractedField ")")
                    return field
                }
            }
            dbgLine("[OCR] 'in the " extractedField "' found but not a known field")
        }
        searchPos := inThePos + 7  ; continue searching for more "in the" occurrences
    }

    dbgLine("[OCR] No field matched")
    return ""
}
...
;   Main: attack+kill+stinger screenshot.
;   Searcher: alert Main (SendSearcherMessage, returns alert msgID), wait until >=1 Main has joined+ready (so the Searcher doesn't leave a server no Main is coming to), then return — the caller leaves + (off-server) waits for all Mains to leave + sends the Passive "done".
HuntVB(field) {
    global AccountMode, currentLink, currentAlertMsgID
    if (AccountMode = "Searcher") {
        currentAlertMsgID := SendSearcherMessage(currentLink, field)
        WaitForMainReady(currentAlertMsgID)
        return
    }
    AttackVicLoop(field)
